...
forward.prop = function(NNModel = NULL, x=NULL){
  
  # NNModel = list generated from NNModel() function above
  # x = matrix of inputs (1 x m) of appropriate dimensions for the NNModel
  ...
  layers = names(NNModel$layers)
  lout = list()
  
  for(l in 1:length(layers)){
    
    if( l == 1 ){
      z = x %*% NNModel$layers[[layers[l]]]$weights
    }else{
      z = lout[[layers[l-1]]] %*% NNModel$layers[[layers[l]]]$weights
    }
    
    if( NNModel$layers[[layers[l]]]$activation == 'linear' ){
      lout[[layers[l]]] = linear(z)
    }else if( NNModel$layers[[layers[l]]]$activation == 'sigmoid' ){
      lout[[layers[l]]] = sigmoid(z)
    }else if( NNModel$layers[[layers[l]]]$activation == 'relu' ){
      lout[[layers[l]]] = relu(z)
    }else if( NNModel$layers[[layers[l]]]$activation == 'tanh' ){
      lout[[layers[l]]] = tanh(z)
    }else if( NNModel$layers[[layers[l]]]$activation == 'softmax' ){
      lout[[layers[l]]] = softmax(z)
    }
  }
...
back.prop = function(NNmodel=NULL, layer.outputs=NULL, output.error=NULL) {
...
train = function(NNModel=NULL, X=NULL, mini.batch.size=NULL, iterations=NULL) {
...
predict = function( NNModel=NULL, X=NULL) {
  ...
  # returns a vector of predictions for each observation in the training set
